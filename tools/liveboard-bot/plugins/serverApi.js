...
export function initServerApi(options = {}) {
    ...
    app.post('/api/server/start', authGuard, async (req, res) => {
        ...
        if (!isAlreadyRunning) {
            isAlreadyRunning = (await isProcessRunningSilent('PalServer-Win64-Shipping-Cmd.exe')) ||
                               (await isProcessRunningSilent('PalServer-Win64-Shipping.exe')) ||
    ...
    app.post('/api/server/stop', authGuard, async (req, res) => {
        ...
        await killProcessSilent('PalServer-Win64-Shipping-Cmd.exe');
    ...
    server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            console.warn(`[Server API] Port ${port} is already in use by another instance or process. Bot will continue running.`);
        } else {
            console.error('[Server API] Server error:', err.message);
        }
    });

    activeServer = { app, server };
    return activeServer;
}
