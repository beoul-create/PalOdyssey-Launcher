    ...
    async function update() {
        ...
        try {
            ...
            if (!state.ServerOnline) {
                const commandChannel = process.env.DISCORD_COMMAND_CHANNEL_ID ? `<#${process.env.DISCORD_COMMAND_CHANNEL_ID}>` : '`#commands`';
                fields.push({
                    name: '⚡ On-Demand Server Boot',
                    value: `Server is currently offline. Anyone can start it instantly by typing \`/server start\` in ${commandChannel}.`,
                    inline: false
                });
            }
