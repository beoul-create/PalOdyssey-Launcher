...
function A.meleeSweep()
  ...
  local now = os.clock()
  if (now - meleeRes.sweepAt) < 3 then return end
...
function A.getEquippedWeapon()
  ...
  if not alive(shooter) then
    shooter = nil
    for _, sc in ipairs(safe(function() return FindAllOf("PalShooterComponent") end) or {}) do
      if alive(sc) and safe(function() return sc:GetOwner() end) == pawn then shooter = sc; break end
    end
  end
